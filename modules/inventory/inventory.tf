...
data "template_file" "tower_block" {
  count = "${var.tower_node_count}"
  template = "$${hostname}"
  vars {
   hostname = "${element(var.tower_host, count.index)}.${var.domain}"
  }
}
...
data "template_file" "ose_inventory_new" {
  ...
  vars {
    ...
    tower_block     =  "${join("\n", data.template_file.tower_block.*.rendered)}"
  }
}
...
data "template_file" "tower_host_file_template" {
  count = "${var.tower_node_count}"
  template = "$${tower_ip} $${tower_host} $${tower_host_domain} "
  vars {
    tower_ip = "${element(var.tower_private_ip, count.index)}"
    tower_host = "${element(var.tower_host, count.index)}"
    tower_host_domain = "${element(var.tower_host, count.index)}.${var.domain}"
  }
}

# Create a installer config file for openshift installation
resource "local_file" "host_file_render" {
  content     = "${join("\n", concat(data.template_file.master_host_file_template.*.rendered,data.template_file.infra_host_file_template.*.rendered,data.template_file.app_host_file_template.*.rendered,data.template_file.master_public_host_file_template.*.rendered,data.template_file.storage_host_file_template.*.rendered,data.template_file.tower_host_file_template.*.rendered))}"
