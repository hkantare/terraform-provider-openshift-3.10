...
resource "null_resource" "pre_install_cluster" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/tmp/scripts/pre_install_steps.sh ${join(",",concat(var.app_private_ip,var.infra_private_ip,var.master_private_ip,var.storage_private_ip,var.tower_private_ip))} ${join(",",concat(var.app_host,var.infra_host,var.master_host,var.storage_host,var.tower_host))} ${var.domain}"
    ]
  }
}
...
resource "null_resource" "post_install_cluster" {
  ...
  provisioner "remote-exec" {
    inline = [
      "/tmp/scripts/post_install_steps.sh ${join(",",var.master_private_ip)} ${join(",",concat(var.app_private_ip,var.infra_private_ip,var.storage_private_ip,var.tower_private_ip))}",
    ]
  }
